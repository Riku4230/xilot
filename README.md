...
Codex app-serverをバックエンドとして、記事の全文翻訳、AIチャット、インフォグラフィック画像生成をサイドパネルで提供します。

## 機能

- **AI翻訳** — X記事の全文を日本語にストリーミング翻訳（スケルトンUI + プログレスバーで進捗表示）
- **スクロール連動** — ブラウザとサイドパネルのスクロールが自動同期。ホバーでハイライト
...
- **セッション管理** — 記事ごとにチャット履歴を自動保存・復元。新規チャット作成・セッション切り替え可能
...
1. X記事のページを開く（例: `x.com/user/status/...` または `x.com/user/article/...`）
